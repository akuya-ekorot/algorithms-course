...
import { type LessonObjective, type CompleteLessonObjective } from "@/lib/db/schema/lessonObjectives";
import { OptimisticAction } from "@/lib/utils";
import { useOptimistic } from "react";

export type TAddOptimistic = (action: OptimisticAction<LessonObjective>) => void;

export const useOptimisticLessonObjectives = (
  lessonObjectives: CompleteLessonObjective[],
  lessons: Lesson[]
) => {
  const [optimisticLessonObjectives, addOptimisticLessonObjective] = useOptimistic(
    lessonObjectives,
    (
      ...
      switch (action.action) {
        ...
        case "update":
          return currentState.map((item) =>
            item.id === data.id ? { ...item, ...optimisticLessonObjective } : item,
          );
        case "delete":
          return currentState.map((item) =>
