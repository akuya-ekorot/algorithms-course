...
import { type CourseObjective, type CompleteCourseObjective } from "@/lib/db/schema/courseObjectives";
import { OptimisticAction } from "@/lib/utils";
import { useOptimistic } from "react";

export type TAddOptimistic = (action: OptimisticAction<CourseObjective>) => void;

export const useOptimisticCourseObjectives = (
  courseObjectives: CompleteCourseObjective[],
  courses: Course[]
) => {
  const [optimisticCourseObjectives, addOptimisticCourseObjective] = useOptimistic(
    courseObjectives,
    (
      ...
      switch (action.action) {
        ...
        case "update":
          return currentState.map((item) =>
            item.id === data.id ? { ...item, ...optimisticCourseObjective } : item,
          );
        case "delete":
          return currentState.map((item) =>
