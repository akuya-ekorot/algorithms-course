...
const LessonObjectiveForm = ({
  ...
  const handleSubmit = async (data: FormData) => {
    ...
    const lessonObjectiveParsed =
      await insertLessonObjectiveParams.safeParseAsync({
        lessonId,
        ...payload,
      });
    ...
    try {
      startMutation(async () => {
        ...
        const error = editing
          ? await updateLessonObjectiveAction({
              ...values,
              id: lessonObjective.id,
            })
      ...
      {editing ? (
        <Button
          ...
          onClick={() => {
            ...
            startMutation(async () => {
              addOptimistic &&
                addOptimistic({ action: 'delete', data: lessonObjective });
              const error = await deleteLessonObjectiveAction(
                lessonObjective.id,
              );
              setIsDeleting(false);
              const errorFormatted = {
