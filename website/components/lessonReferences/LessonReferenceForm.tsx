...
import { type LessonReference, insertLessonReferenceParams } from "@/lib/db/schema/lessonReferences";
import {
...
const LessonReferenceForm = ({
  ...
  const handleSubmit = async (data: FormData) => {
    ...
    const lessonReferenceParsed = await insertLessonReferenceParams.safeParseAsync({ lessonId, ...payload });
    ...
    try {
      startMutation(async () => {
        ...
        const error = editing
          ? await updateLessonReferenceAction({ ...values, id: lessonReference.id })
      ...
      {editing ? (
        <Button
          ...
          onClick={() => {
            ...
            closeModal && closeModal();
            startMutation(async () => {
              addOptimistic && addOptimistic({ action: "delete", data: lessonReference });
              const error = await deleteLessonReferenceAction(lessonReference.id);
              setIsDeleting(false);
              const errorFormatted = {
                error: error ?? "Error",
