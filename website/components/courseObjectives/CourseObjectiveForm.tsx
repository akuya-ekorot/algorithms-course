...
const CourseObjectiveForm = ({
  ...
  const handleSubmit = async (data: FormData) => {
    ...
    const courseObjectiveParsed =
      await insertCourseObjectiveParams.safeParseAsync({
        courseId,
        ...payload,
      });
    ...
    try {
      startMutation(async () => {
        ...
        const error = editing
          ? await updateCourseObjectiveAction({
              ...values,
              id: courseObjective.id,
            })
      ...
      {editing ? (
        <Button
          ...
          onClick={() => {
            ...
            startMutation(async () => {
              addOptimistic &&
                addOptimistic({ action: 'delete', data: courseObjective });
              const error = await deleteCourseObjectiveAction(
                courseObjective.id,
              );
              setIsDeleting(false);
              const errorFormatted = {
