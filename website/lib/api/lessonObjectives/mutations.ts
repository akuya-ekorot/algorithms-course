...
export const createLessonObjective = async (lessonObjective: NewLessonObjectiveParams) => {
  const newLessonObjective = insertLessonObjectiveSchema.parse(lessonObjective);
  try {
    const [l] =  await db.insert(lessonObjectives).values(newLessonObjective).returning();
...
export const updateLessonObjective = async (id: LessonObjectiveId, lessonObjective: UpdateLessonObjectiveParams) => {
...
export const deleteLessonObjective = async (id: LessonObjectiveId) => {
  const { id: lessonObjectiveId } = lessonObjectiveIdSchema.parse({ id });
  try {
    const [l] =  await db.delete(lessonObjectives).where(eq(lessonObjectives.id, lessonObjectiveId!))
    .returning();
    return { lessonObjective: l };
  } catch (err) {
    const message = (err as Error).message ?? "Error, please try again";
