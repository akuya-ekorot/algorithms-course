...
export const createCourseObjective = async (courseObjective: NewCourseObjectiveParams) => {
  const newCourseObjective = insertCourseObjectiveSchema.parse(courseObjective);
  try {
    const [c] =  await db.insert(courseObjectives).values(newCourseObjective).returning();
...
export const updateCourseObjective = async (id: CourseObjectiveId, courseObjective: UpdateCourseObjectiveParams) => {
...
export const deleteCourseObjective = async (id: CourseObjectiveId) => {
  const { id: courseObjectiveId } = courseObjectiveIdSchema.parse({ id });
  try {
    const [c] =  await db.delete(courseObjectives).where(eq(courseObjectives.id, courseObjectiveId!))
    .returning();
    return { courseObjective: c };
  } catch (err) {
    const message = (err as Error).message ?? "Error, please try again";
