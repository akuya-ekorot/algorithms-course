...
export const createLessonReference = async (lessonReference: NewLessonReferenceParams) => {
  const newLessonReference = insertLessonReferenceSchema.parse(lessonReference);
  try {
    const [l] =  await db.insert(lessonReferences).values(newLessonReference).returning();
...
export const updateLessonReference = async (id: LessonReferenceId, lessonReference: UpdateLessonReferenceParams) => {
...
export const deleteLessonReference = async (id: LessonReferenceId) => {
  const { id: lessonReferenceId } = lessonReferenceIdSchema.parse({ id });
  try {
    const [l] =  await db.delete(lessonReferences).where(eq(lessonReferences.id, lessonReferenceId!))
    .returning();
    return { lessonReference: l };
  } catch (err) {
    const message = (err as Error).message ?? "Error, please try again";
